...
**Note:** Make sure all Genuin SDK modules are added with the same version (2.0.2) for compatibility.
...
func loadEmbed() {
    ...
    let attributes = GenuinEmbedAttributes(embedId: videoEmbedId,
                                           viewController: self)
    embedView?.initWith(attributes: attributes)
    embedView?.fetchDataAndLoad()
}
...
self.viewIHeartVerticalCarousel.addSubview(placementView)
placementView.fetchDataAndLoad()
...
func loadPlacement() {
    ...
    viewPlacement.initWith(attributes: attributes)
    viewPlacement.fetchDataAndLoad()
}
...
placementView.fetchDataInBackground { success in
    if success {
        self.carouselContainer1.addSubview(placementView)
        placementView.load()
    }
}
...
`ARRAY_OF_ASSOCIATED_DOMAINS_HANDLED_BY_YOUR_APP` accepts associcated domains supported by your apps. You can pass comma seperated domains like e.g. ["apple.com", "https://apple.com", "www.apple.com", "applinks:apple.com"]
