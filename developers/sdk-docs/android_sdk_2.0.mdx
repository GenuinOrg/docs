...
| -------- | ------------------- | -------- | 
| Core  | Includes the core functionality of the Genuin Ecosystem: Embeds and User authentication. | com.begenuin:core:2.1.0.4 |
| UI  | Additional UI flows like comments, search, deep-links, notifications, group/community detail etc. | com.begenuin:ui:2.1.0.4 |
| Camera  | Camera flows for video content creation. | com.begenuin:camera:2.1.0.4 |
| AI  | Additional flows for AI assisted community/group creation. | com.begenuin:ai:2.1.0.4 |

## Requirements

| Platform | Minimum SDK Version | Language | Supported Orientations | Supported Destinations | Minimum AGP Version |
| -------- | ------------------- | -------- | ---------------------- | ---------------------- | -------------------- |
| Android  | 24                  | Java/Kotlin    | Portrait               | Mobile           | 8.6.0                |
...
val genuinVersion = "2.1.0.4"
implementation("com.begenuin:core:$genuinVersion")
```

```kotlin Groovy-build.gradle
def genuinVersion = "2.1.0.4"
implementation "com.begenuin:core:$genuinVersion"
```
</CodeGroup>
...
Note: All modules must use a consistent version format — 2.1.0.4.
...
```
```kotlin Embed with Customization & SSO
...
```kotlin Embed With Customization and SSO
...
<Note> **Note**: Placement Container height must be fixed or match_parent. It can not be wrap_content else placement will not appear. </Note>

<CodeGroup>

```kotlin PlacementActivity.kt (With SSO)
...
```kotlin PlacementActivity.kt (Without SSO)
...
<Note> **Note**: To initialise the Placement you need to add your activity context `(setActivity(activityContext))` in which you want the placement. To auto login in the SDK,  you shall pass "YOUR_SSO_TOKEN"`(setSSOToken("YOUR_SSO_TOKEN"))` in order to implement Placement with SSO in your app. </Note>
...
```Kotlin activity_full_placement.xml
...
<Note> **Note**: You don't need to call the below method if you have implemented the Embed/Placement With SSO already. </Note>
