...
```html
window.genuinAuth = ({ path, action }) => {
  // You can trigger your own login mechanism
  console.log('Custom auth triggered:', action, path);

  // Option 1: Use a custom redirect function
  redirectToLogin(path);

  // Option 2: Directly set the login URL
  // window.location.href = `https://auth.pocketfilms.in/login?redirect=${encodeURIComponent(path)}`;
};
```
