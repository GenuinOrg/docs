...
auth callback will pass certain parameters to perform necessary actions after the user is successfully logged in or redirected — based on whichever flow you implement.
For example: we pass returnQueryParams, which contains a URL with query parameters indicating the context in which the auth callback was triggered.
This helps you determine what action to take.

Example: if a user is trying to comment on a post and the auth callback is triggered, the URL will have a query param like action=comment.
You can then use this action param and pass it to the embeds to perform the corresponding operation.
...
window.genuinAuth = (data) => {
  ...
  // data.returnQueryParams contains the URL with query parameters
