...
target 'YourProject' do
  ...
  pod "GenuinSDK", :git => "https://bitbucket.org/genuindev/genuin_sdk_ios.git", :tag => "1.0.15"
...
### If you are using `SceneDelegate` method follow the instructions:
...
### If you are using `AppDelegate` method follow the instructions:
...
To present feed from your UIViewController by click on UIButton:
...
To fetch the Embed you need to add `YOUR_EMBED_ID`, `YOUR_CONTAINER_VIEW` in which you want the embed, and `YOUR_VIEW_CONTROLLER` containing your container view, while calling the below function.

<CodeGroup>

```swift ViewController.swift
GenuinSDK.shared.initializeEmbed(embedId: "YOUR_EMBED_ID", containerView: <YOUR_CONTAINER_VIEW>, viewConroller: <YOUR_VIEW_CONTROLLER>)

```

</CodeGroup>

**interactionDeepLink** = This is an optional parameter. You can pass a deeplink URL in this parameter. If a deeplink URL is given then all the interaction/clicks in the full screen view will redirect to the deeplink URL given. If not passed then the regular flow will work.

**Note**: The `interactionDeepLink` must be a valid url else user will not be redirected.

<CodeGroup>

```swift ViewController.swift
GenuinSDK.shared.initializeEmbed(embedId: "YOUR_EMBED_ID", containerView: <YOUR_CONTAINER_VIEW>, viewConroller: <YOUR_VIEW_CONTROLLER>, interactionDeepLink: "YOUR_INTERACTION_DEEPLINK")
...
## Handling Notifications/Push Notifications

### Firebase Setup

1. Create an app in your firebase console ( add appId , bundle id , APNS key ) 
...
import FirebaseMessaging // If you are using firebase then only import this library
import GenuinSDK
...
func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
	//Assign APNS token and notify sdk
        Messaging.messaging().apnsToken = deviceToken // If you are not using firebase then remove this code of line
        GenuinSDK.shared.application(application, didRegisterForRemoteNotificationsWithDeviceToken: deviceToken, isFCMIntegrated: true) //pass isFCMIntegrated == false if you don’t use firebase push notifcation
...
// If you are using firebase then only use the below code
	 //configure firebase
...
5. Place this code to take the user permission for Push Notification
...
6. Extend `MessagingDelegate` and Add follwing method:
...
7. To Handle the Push Notification Redirection whenever the user taps, follow the below code
...
func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
	 ...
	 //Get userinfo
        if let userInfo = response.notification.request.content.userInfo as? [String: AnyObject]{
	     //Check sdk will handle the notification or not 
            guard GenuinSDK.shared.willHandleNotification(userInfo: userInfo) else { return }
            
            Messaging.messaging().appDidReceiveMessage(userInfo)  // If you are not using firebase then remove this code of line
            //notify sdk
...
8. To configure notification UI
...
9. If you want to handle the notification when app is opened then follow the below code
