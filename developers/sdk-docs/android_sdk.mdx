...
<Note> **Note**: Currently minimum SDK version of our SDK is 24 but if you want to use our SDK in version below 24, use the below code snippet in the App level manifest file. </Note>
...
implementation("org.bitbucket.genuindev:genuin_android_sdk:1.1.4@aar"){
        isTransitive=true
    }
```
...
2. **uniqueId** = This is an optional parameter. This uniqueId is used when we need to display same embed in multiple/same screen. We need to provide distinct uniqueId for the same embedId in multiple/same screen.
...
<Note> **Note**: Need to call this function everytime your app receives any deeplink. This method will handle deeplink if its our deeplink otherwise it will ignore that. </Note>
...
2. Download the `google-services.jsonfile` and add it into your app.
...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (GenuinSDK.willHandleForegroundNotification(data)) {
            ...
            GenuinSDK.handleForegroundNotification(
                context = this,
                data = data,
                message = message,
...
You will get the notification payload in your launcher activity.
