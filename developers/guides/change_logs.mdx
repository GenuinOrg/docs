...
<Tabs>
  <Tab title="Android">
    <AccordionGroup>
      ...
      <Accordion title="2024-24"> {/* Year Accordion */}
        <AccordionGroup>
...
<Tabs>
  <Tab title="Android 2.0">
	...
	<Accordion title="2026"> {/* Year Accordion */}
        <AccordionGroup>
          <Accordion title="february">
            **V 2.1.1**  
            1. Dynamic Placement Capabilities
					- Introduce Placement v2 configuration in Android SDK.
					- Support dynamic layouts including Feed View and N×N Grid View.
					- Enable min–max size configuration for card views.
					- Allow placement behavior to adapt based on configuration from BCC.

			2. Backward Compatibility for the SDKs
					- Ensure Placement v2 works seamlessly with older SDK integrations.
					- Maintain support for existing placement configurations without breaking changes.
					- Handle fallback logic when newer configuration fields are unavailable.

			3. Link Configurations via BCC
					- Enable link handling based on BCC configuration.
					- Support blocking or allowing links as per brand/network rules.
					- Ensure consistent link behavior across feed, grid, and placement views.

			4. Ad Improvements
					- Enhance ad serving logic.
					- Improve overall ad performance, reliability, and tracking.
					- Support advanced placement-based ad behavior.

			5. Observability & Analytics
					- Add observability events in Android SDK.
					- Track placement lifecycle, rendering, interactions, and errors.
					- Improve visibility into SDK behavior for debugging and analytics.

			6. Bug Fixes & UX Enhancements
